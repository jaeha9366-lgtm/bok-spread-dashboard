...
<body>
    ...
    <script>
        ...
        function toggleDataset(idx) {{
            const isHidden = myChart.getDatasetMeta(idx).hidden;
            myChart.getDatasetMeta(idx).hidden = !isHidden;
            myChart.update();
            
            if (activeChartMode === 'yields') {{
                hiddenDatasets.yields[idx] = !hiddenDatasets.yields[idx];
                document.getElementById(`legend-yields-${{idx}}`).classList.toggle('disabled', hiddenDatasets.yields[idx]);
            }} else {{
                hiddenDatasets.spreads[idx] = !hiddenDatasets.spreads[idx];
                document.getElementById(`legend-spreads-${{idx}}`).classList.toggle('disabled', hiddenDatasets.spreads[idx]);
            }}
        }}
